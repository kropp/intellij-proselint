...
class ProselintAnnotator : ExternalAnnotator<ProselintAnnotatorInitialInfo, ProselintResult>() {
  override fun collectInformation(file: PsiFile) = ProselintAnnotatorInitialInfo(file.virtualFile.path)
  ...
  override fun doAnnotate(collectedInfo: ProselintAnnotatorInitialInfo) = try {
    val output = ExecUtil.execAndGetOutput(GeneralCommandLine("proselint", "--json", collectedInfo.filepath))
